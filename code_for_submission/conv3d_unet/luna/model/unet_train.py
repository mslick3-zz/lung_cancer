...
def unet_fit(name, start_t, end_t, start_v, end_v, check_name = None):
    ...
    callbacks = [EarlyStopping(monitor='val_loss', patience = 15, 
                                   verbose = 1),
    ModelCheckpoint(luna_unet_model + '/{}.h5'.format(name), 
                        monitor='val_loss', 
    ...
    model.fit_generator(generate_train(start_t, end_t), nb_epoch = 5, verbose = 1, 
...
src = config.get('main', 'luna_processed_image_output')
...
unet_fit('3DUNet_genfulldata', 0, 551, 551, 603)
